...
fn execution_request(symbol: &str, sector_tag: &str, entry_date: &str, leave_open: bool) -> Value {
    let execution_trades = if leave_open {
        json!([
            {
    ...
    json!({
        ...
        "args": {
            ...
            "actual_exit_date": if leave_open { "" } else { "2025-09-29" },
            "actual_exit_price": if leave_open { 0.0 } else { 66.10 },
            "exit_reason": if leave_open { "" } else { "take_profit" },
            "execution_trades": execution_trades,
            "execution_record_notes": ["写入 runtime，供账户层自动回接"],
            "created_at": if leave_open { "2026-04-10T15:00:00+08:00" } else { "2026-04-10T15:10:00+08:00" }
        }
    })
}
