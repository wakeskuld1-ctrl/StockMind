...
| 2026-04-22 | P16 freezes governed P15 apply truth into one explicit execution-status artifact without becoming reconciliation | keep downstream lifecycle work on a named status document instead of rebuilding execution truth from raw apply rows while preserving failed and skipped row semantics explicitly | active | `docs/plans/2026-04-22-p16-portfolio-execution-status-bridge-design.md`, `src/ops/security_portfolio_execution_status_bridge.rs`, `tests/security_portfolio_execution_status_bridge_cli.rs` |
| 2026-04-22 | capital-source validation stays on `raw flow -> standalone snapshot -> standalone audit` before any Nikkei merge | keep JPX/MOF capital-source factors explainable and independently auditable while real JPX coverage is still thin; do not merge into `security_feature_snapshot`, modify `security_scorecard_training`, or trigger a full Nikkei retrain in this round | active | `docs/plans/2026-04-22-capital-source-factor-audit.md`, `docs/plans/2026-04-22-nikkei-capital-source-factors.md`, `src/ops/security_capital_source_factor_snapshot.rs`, `src/ops/security_capital_source_factor_audit.rs` |
| 2026-04-22 | the current capital-source line is phase-closed at standalone verification for this round | prevent thin-sample follow-up pressure from being mistaken for approval to continue business logic; any future JPX sample expansion, audit-method change, or training merge must start from a new approved design rather than implicit continuation | active | `docs/plans/2026-04-22-capital-source-factor-audit.md`, `docs/handoff/CURRENT_STATUS.md`, `docs/handoff/HANDOFF_ISSUES.md` |
