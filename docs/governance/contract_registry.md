...
| `security_nikkei_etf_position_signal` | `SecurityNikkeiEtfPositionSignalRequest` | `SecurityNikkeiEtfPositionSignalResult` | side-effect-free daily Nikkei ETF target-position signal; it trades the ETF target only, uses component stocks only as weighted breadth evidence, reads optional volume proxy rows for 3D-vs-previous-20D breakout confirmation, rejects future-label leakage, and requires an explicit `nikkei_v3_hgb_adjustment.v1` daily artifact for `v3_hgb` mode instead of guessing HGB state | `src/ops/security_nikkei_etf_position_signal.rs` |
